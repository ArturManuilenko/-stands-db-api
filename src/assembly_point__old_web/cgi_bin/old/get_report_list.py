...
add_css_file('/result_table_minimal_black.css')
print_head("Портал тестовых стендов Nero Electronics - перечень проверок")
...
if len(stends):
    ...
    cl.start_command('report_condition', dt_start=dt_from, dt_end=dt_to, stends=stends, report=False)
    request_id = cl.uuid()
    data = ''' {"a[]": "0|reportlist|0"} '''
